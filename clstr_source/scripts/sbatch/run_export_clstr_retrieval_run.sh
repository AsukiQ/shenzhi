...
# Default ToolRet eval files:
#   data/toolret_eval/queries.jsonl
#   data/toolret_eval/skills.jsonl
#   data/toolret_eval/qrels.jsonl
TOOLRET_DATA_DIR=${TOOLRET_DATA_DIR:-data/toolret_eval}
QUERIES_PATH=${QUERIES_PATH:-${TOOLRET_DATA_DIR}/queries.jsonl}
SKILLS_PATH=${SKILLS_PATH:-${TOOLRET_DATA_DIR}/skills.jsonl}
QRELS_PATH=${QRELS_PATH:-${TOOLRET_DATA_DIR}/qrels.jsonl}
OUTPUT_DIR=${OUTPUT_DIR:-outputs/toolret_eval/clstr_retrieval}
...
RUN_EVAL=${RUN_EVAL:-1}
BENCHMARK=${BENCHMARK:-toolret}
RUN_PREFLIGHT_AUDIT=${RUN_PREFLIGHT_AUDIT:-1}
TOOLRET_AUDIT_STATUS=${TOOLRET_AUDIT_STATUS:-not_run}

mkdir -p "${OUTPUT_DIR}"

if [[ "${RUN_PREFLIGHT_AUDIT}" = "1" ]]; then
  "${PYTHON_BIN}" scripts/audit_toolret_eval_data.py \
    --data_dir "${TOOLRET_DATA_DIR}" \
    --output_path "${OUTPUT_DIR}/toolret_eval_audit.json" \
    --fail_on_action_required | tee "${OUTPUT_DIR}/toolret_eval_audit_stdout.json"
  TOOLRET_AUDIT_STATUS=ok
  echo "TOOLRET_AUDIT_STATUS=${TOOLRET_AUDIT_STATUS}"
fi
