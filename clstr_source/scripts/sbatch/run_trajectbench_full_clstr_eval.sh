...
OUTPUT_DIR=${OUTPUT_DIR:-outputs/trajectbench_full_clstr_eval/full_$(date +%Y%m%d_%H%M%S)}
...
MAX_UPDATES=${MAX_UPDATES:-0}
BATCH_SIZE=${BATCH_SIZE:-8}
EVAL_INTERVAL=${EVAL_INTERVAL:-1000000}
EVAL_ROWS=${EVAL_ROWS:-1000000}
...
ALLOWED_BENCHMARKS=${ALLOWED_BENCHMARKS:-traject_bench}
BENCHMARK_CAPS=${BENCHMARK_CAPS:-traject_bench=-1}
EVAL_BENCHMARK_CAPS=${EVAL_BENCHMARK_CAPS:-traject_bench=-1}
...
ONLINE_MEMORY_NEXT_SKILL_BONUS=${ONLINE_MEMORY_NEXT_SKILL_BONUS:-0.25}
ONLINE_MEMORY_EXACT_TRANSITION_BONUS=${ONLINE_MEMORY_EXACT_TRANSITION_BONUS:-3.0}
ONLINE_MEMORY_MODE=${ONLINE_MEMORY_MODE:-inventory_remaining}
...
ONLINE_MEMORY_AUTO_GATE=${ONLINE_MEMORY_AUTO_GATE:-1}
ONLINE_MEMORY_GATE_SCOPE=${ONLINE_MEMORY_GATE_SCOPE:-source_benchmark}
ONLINE_MEMORY_GATE_EVAL_ROWS=${ONLINE_MEMORY_GATE_EVAL_ROWS:-1024}
...
if [[ -s "${ROUTING_CHECKPOINT_PATH}" ]]; then
  ...
  echo "ERROR: TrajectBench full CLSTR eval requires retriever checkpoint: ${ROUTING_CHECKPOINT_PATH}" >&2
  exit 2
fi
...
if [[ -s "${HEAD_CHECKPOINT_PATH}" ]]; then
  ...
  echo "ERROR: TrajectBench full CLSTR eval requires Stage2/head checkpoint: ${HEAD_CHECKPOINT_PATH}" >&2
  exit 2
fi
...
printf '[trajectbench-clstr-eval] output_dir=%s\n' "${OUTPUT_DIR}"
printf '[trajectbench-clstr-eval] route_scorer=%s transition_scoring_mode=%s\n' "${ROUTE_SCORER}" "${TRANSITION_SCORING_MODE}"
printf '[trajectbench-clstr-eval] routing_checkpoint=%s\n' "${ROUTING_CHECKPOINT_PATH}"
printf '[trajectbench-clstr-eval] head_checkpoint=%s\n' "${HEAD_CHECKPOINT_PATH}"

"${PYTHON_BIN}" "${ARGS[@]}" | tee "${OUTPUT_DIR}/eval_stdout.json"
