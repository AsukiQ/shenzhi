#!/bin/bash
#SBATCH --job-name=tau2_skillrouter
#SBATCH -p gpu_a800
#SBATCH --gpus=1
...
OUTPUT_DIR=${OUTPUT_DIR:-outputs/tau2_skillrouter_frozen_eval/smoke_${SLURM_JOB_ID:-manual}}
...
args=(
  "${PYTHON_BIN}" scripts/run_tau2_skillrouter_frozen_eval.py
  --data_root "${DATA_ROOT}"
...
printf '[tau2-skillrouter-eval] output_dir=%s\n' "${OUTPUT_DIR}"
printf '[tau2-skillrouter-eval] domains=%s max_tasks_per_domain=%s max_eval_rows=%s\n' "${DOMAINS}" "${MAX_TASKS_PER_DOMAIN:-ALL}" "${MAX_EVAL_ROWS:-ALL}"
printf '[tau2-skillrouter-eval] adapter_checkpoint_path=%s\n' "${ADAPTER_CHECKPOINT_PATH:-none}"
