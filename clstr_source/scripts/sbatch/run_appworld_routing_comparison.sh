...
OUTPUT_DIR=${OUTPUT_DIR:-outputs/appworld_routing_comparison}

if [ "$#" -eq 0 ]; then
  echo "ERROR: pass one or more routing report.json paths as arguments" >&2
  exit 1
fi

"${PYTHON_BIN}" scripts/build_appworld_routing_comparison.py \
  --reports "$@" \
