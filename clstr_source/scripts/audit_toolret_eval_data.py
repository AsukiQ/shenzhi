...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit normalized ToolRet eval data before CLSTR run export/evaluation."
    )
    parser.add_argument("--data_dir", default="data/toolret_eval")
    parser.add_argument("--output_path")
    parser.add_argument("--fail_on_action_required", action="store_true")
    args = parser.parse_args()

    report = audit_toolret_eval_data(
