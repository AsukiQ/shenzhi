#!/bin/bash
#SBATCH --job-name=shenzhi_s2_eval
#SBATCH -p gpu_a800,gpu_h100,gpu_h200
#SBATCH --gpus=1
#SBATCH --cpus-per-task=6
#SBATCH --time=02:00:00
#SBATCH --output=/data/home/scyb713/run/xzf/AAAI/autodl-tmp/shenzhi/logs/stage2-policy-eval-%j.out
set -euo pipefail
...
"$CONDA_PREFIX/bin/python" evaluate_stage2_policy.py \
  ...
  --clstr-source "$CLSTR" \
  --dev-trajectories "$ROOT/derived/action_vnext_stage2_v1/trajectory_dev.jsonl" \
  --device cuda \
  --output "$ROOT/outputs/stage2_policy_eval.json"
