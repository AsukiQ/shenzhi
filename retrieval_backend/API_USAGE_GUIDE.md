...
多步接口采用“约束优先、关系按顺序扩展”的混合策略：年份、会议、作者、关键词和主题等明确条件会在初始召回及每次扩展后作为硬过滤；查询中的“同作者/相关主题/同会议”等关系短语则按出现顺序触发图谱扩展。模型未启用时使用确定性的规则策略，模型只用于可选的动作选择增强。
...
1. 所有接口统一使用 `http://47.110.47.12`，不要使用服务器内部地址 `172.19.0.1:8080`。
2. `paper_id` 必须使用检索接口返回的原始值，并在 URL 中编码。
3. 普通检索使用 `/api/retrieval/search`，不要使用 `/api/kg/search` 替代。
4. 图谱查询建议设置 `depth=1`，避免返回过多数据。
5. 检索结果可能为空，部分论文也可能缺少作者、DOI、PDF 或引用数据。
6. 当前论文检索主要基于标题、摘要和元数据，不代表已经检索完整 PDF 正文。
7. 当前服务使用 HTTP，外部正式生产环境接入前建议增加 HTTPS 和访问认证。
