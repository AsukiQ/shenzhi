...
STAGE0_OUTPUT=${STAGE0_OUTPUT:-$SHENZHI_ROOT/outputs/action_vnext_stage0_skillrouter_smoke}
STAGE2_DATA=${STAGE2_DATA:-$SHENZHI_ROOT/derived/action_vnext_stage2_v1}
STAGE0_CHECKPOINT=${STAGE0_CHECKPOINT:-$STAGE0_OUTPUT/checkpoints/clstr_vnext_stage0-step20.pt}
OUTPUT_DIR=${OUTPUT_DIR:-$SHENZHI_ROOT/outputs/action_vnext_stage2_skillrouter_smoke}

for required in \
  "$STAGE0_CHECKPOINT" \
  "$STAGE0_OUTPUT/selected_skills.jsonl" \
  "$STAGE2_DATA/data_contract.json" \
  "$STAGE2_DATA/trajectory_train.jsonl" \
  "$STAGE2_DATA/trajectory_dev.jsonl"; do
...
"$PYTHON_BIN" scripts/run_clstr_vnext_stage2_train.py \
  ...
  --inventory_catalogs_path "$STAGE2_DATA/../action_vnext_stage0_v1/inventory_catalogs.jsonl" \
